    ...
    <div className="mx-auto w-full max-w-2xl px-6 py-14">
      ...
      <section>
        ...
        <button
          ...
          className="mt-4 rounded-md bg-[var(--accent)] px-6 py-3 font-medium text-[var(--on-accent)] transition-[background-color,transform] duration-[var(--duration-fast)] hover:bg-[var(--accent-hover)] active:scale-[0.98] disabled:opacity-40"
        >
      ...
      {result && (
        <article className="launch-card mt-14 overflow-hidden">
          <div className="px-8 pt-7 sm:px-10">
            {/* meta row */}
            <div
              className="reveal flex items-baseline justify-between"
              style={delay()}
            >
              <span className="label text-[var(--card-accent)]">carretai</span>
              <span className="label text-[var(--card-muted)]">
                pitch&nbsp;2:00 · build night bogotá
              </span>
            </div>

            {/* name — the identity */}
            <div className="reveal mt-8" style={delay()}>
              <p className="display text-5xl leading-none [text-wrap:balance]">
                {result.names[0].name}
              </p>
              <p className="mt-3 text-lg leading-snug text-[var(--card-muted)] [text-wrap:pretty]">
                {result.oneliner}
              </p>
              {result.names.length > 1 && (
                <p className="label mt-4 text-[var(--card-muted)]">
                  alt:&nbsp;
            ...
            <div className="reveal" style={delay()}>
              ...
              <p className="display-bold mt-3 text-3xl leading-tight [text-wrap:balance]">
                {result.hero.headline}
              </p>
              <p className="mt-2 leading-relaxed text-[var(--card-muted)] [text-wrap:pretty]">
                {result.hero.subhead}
              </p>
            </div>
              ...
              <ul className="mt-3 space-y-2">
                {result.bullets.map((b) => (
                  <li key={b} className="flex gap-3 leading-relaxed">
                    <span aria-hidden className="text-[var(--card-accent)]">→</span>
                    <span>{b}</span>
                  </li>
                ))}
              </ul>
              ...
              <ol className="mt-4 space-y-5">
                {PITCH_STEPS.map((s) => (
                  <li key={s.key} className="grid grid-cols-[3.5rem_1fr] gap-4">
                    <span className="pt-0.5 font-mono text-sm tabular-nums text-[var(--card-accent)]">
                      {s.time}
                    </span>
                    <div>
          ...
          {/* perforated stub */}
          <div className="perforation reveal mt-9" style={delay()}>
            <div className="flex items-baseline justify-between px-8 py-5 sm:px-10">
              <span className="label text-[var(--card-muted)]">
                hecho con carretai
              </span>
              <span className="label text-[var(--card-accent)]">
